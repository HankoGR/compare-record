...
def update_dashboard(n_clicks, first, last, category, gender, result):
    ...
    # Determine position within current filtered group
    full_ranking = best[(best['Category'] == category) & (best['Gender'] == gender)]
    full_ranking = full_ranking.sort_values('Result').reset_index(drop=True)
    full_ranking['Overall_Pos'] = full_ranking.index + 1

    merged = merged.merge(full_ranking[['Full_Name', 'Overall_Pos']], on='Full_Name', how='left')
    merged['Ranked_Name'] = merged['Overall_Pos'].astype(str) + '. ' + merged['Full_Name']

    # Highlight user result with blue if not already in top 6
    bar_colors = [
        'dodgerblue' if name == full_name else 'green' if x <= 1.5 else 'gold' if x <= 3 else 'crimson'
        for name, x in zip(merged['Full_Name'], merged['%_Diff'])
    ]
    ...
    fig.update_layout(
        ...
        yaxis_title="Position – Athlete",
        yaxis=dict(autorange='reversed'),
        plot_bgcolor='white'
    )

    # Calculate full ranking
    position = full_ranking[full_ranking['Full_Name'] == full_name]['Overall_Pos'].values[0]
